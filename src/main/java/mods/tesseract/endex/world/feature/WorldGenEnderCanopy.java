...
import mods.tesseract.endex.utils.MathUtils;
...
public class WorldGenEnderCanopy extends WorldGenAbstractTree {
    private static final IBlockState LOG = BlockInit.WYRMWOOD_LOG.getDefaultState();
    private static final IBlockState LEAF = BlockInit.WYRMWOOD_LEAVES.getDefaultState();
    private static final IBlockState END_GRASS = BlockInit.STYGIAN_GRASS.getDefaultState();
    private static final IBlockState CREEPER = BlockInit.STYGIAN_CREEPER.getDefaultState();
    ...
    public boolean isValidGenLocation(World world, BlockPos pos, int trunkHeight) {
        ...
        for (BlockPos trunkBaseBlock : BlockPos.getAllInBoxMutable(pos.add(-5, 0, -5), pos.add(5, 2, 5))) {
            ...
            if (state != END_GRASS && state != Blocks.END_STONE.getDefaultState() && state.getBlock() != BlockInit.WYRMWOOD_SAPLING && !state.getBlock().isReplaceable(world, pos) && !isReplaceable(world, trunkBaseBlock, state)) {
    ...
    private void addHangingVine(World world, BlockPos pos, PropertyBool prop, int length) {
        this.setBlockAndNotifyAdequately(world, pos, CREEPER.withProperty(prop, true));

        for (BlockPos blockpos = pos.down(); world.isAirBlock(blockpos) && length > 0; length--) {
            this.setBlockAndNotifyAdequately(world, blockpos, CREEPER.withProperty(prop, true));
...
//				}
    ...
    private void placeLogAt(World worldIn, BlockPos pos) {
        this.setBlockAndNotifyAdequately(worldIn, pos, worldIn.rand.nextInt(20) == 0 ? BlockInit.WYRMWOOD_INFESTED.getDefaultState() : LOG);
    }

    private void placeLeafAt(World worldIn, BlockPos pos) {
        IBlockState state = worldIn.getBlockState(pos);

        if (state.getBlock().isAir(state, worldIn, pos) || state.getBlock().isLeaves(state, worldIn, pos) || state == CREEPER) {
